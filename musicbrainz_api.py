...
def get_artist_info_smart(recording_mbids):
    ...
    if total == 0:
        ...
        # For larger numbers, use the appropriate batch processing method
        print(f"🚀 Using batch processing for {total} recordings")
        if LOCAL_MB_MIRROR:
            # Use threading for local mirrors since there are no rate limits
            print(f"🚀 Using concurrent requests for {total} recordings (local mirror: {MB_MIRROR})")
            return _get_artist_info_concurrent(recording_mbids, max_workers=10)
        else:
